...
def phase_change_sinusoid(n: np.ndarray, A: float, Omega0: float, phi: float, phi0: float) -> np.ndarray:
    """Computes the discrete-time sinusoid with an added phase phi0."""
...
def main():
    ...
    print(f"[Part A] MSE between time-shifted and equivalent phase-changed: {err_A:.6e}")
    ...
    ax1.legend()
    plt.title("Time Shift vs Equivalent Phase Change")
    fig1.tight_layout()

    # -----------------------------
    # Part B: Phase Change -> Equivalent Time Shift?
    # -----------------------------
    ...
    print(f"[Part B] Best matching integer shift in [{k_min},{k_max}] is k={best_k} with MSE={best_err:.6f}")
    ...
    ax2.legend()
    plt.title("Arbitrary Phase Change vs Best Integer Time Shift")
    fig2.tight_layout()
