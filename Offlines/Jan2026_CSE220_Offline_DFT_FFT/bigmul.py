...
def from_limbs(sign, limbs, base_digits=BASE_DIGITS):
    ...
    """
    base = 10 ** base_digits
    # Work with a Python list for carry propagation (may need to extend)
    limbs_list = [int(v) for v in limbs]

    carry = 0
    for i in range(len(limbs_list)):
        total = limbs_list[i] + carry
        limbs_list[i] = total % base
        carry = total // base

    # If there's remaining carry, extend
    while carry > 0:
        limbs_list.append(carry % base)
        carry //= base

    # Strip trailing zeros (most significant end, since little-endian)
    while len(limbs_list) > 1 and limbs_list[-1] == 0:
        limbs_list.pop()

    # Build the decimal string (big-endian)
    # Most significant limb doesn't need zero-padding
    result = str(limbs_list[-1])
    for i in range(len(limbs_list) - 2, -1, -1):
        result += str(limbs_list[i]).zfill(base_digits)

    # Handle zero case
    if result == '' or result == '0' * len(result):
        return '0'

    # Attach sign (never attach to "0")
    if result == '0':
        return '0'
...
def multiply_transform(a, b, engine):
    ...
    n = len(a)
    q = len(b)
    linear_len = n + q - 1  # Minimum length to avoid circular aliasing

    if engine.name == "arbitrary":
        N = linear_len  # ArbitraryLengthFFT handles any N -- no padding needed
    else:
        # Both DFT and FFT: pad to next power of two.
        # The DFT works at any length, but padding to a power of two matches
        # the expected output and wastes only a constant factor.
    ...
    # Real part and round to nearest integer
    result = np.round(c.real).astype(np.int64)
...
def multiply_schoolbook(a, b):
    ...
    # Use NumPy convolution-style schoolbook multiply
    for i in range(n):
        result[i:i + q] += a[i] * b
    return result
...
def multiply(text_a, text_b, method):
    ...
    if method == "schoolbook":
        conv = multiply_schoolbook(limbs_a, limbs_b)
        N = len(limbs_a) + len(limbs_b) - 1
    else:
        conv, N = multiply_transform(limbs_a, limbs_b, engine)

    # Convert result limbs back to decimal string (carry propagation inside)
    product_str = from_limbs(result_sign, conv)
...
def run_single(path, method, out_dir):
    ...
    # Verification using Python's big integers (allowed only here)
    expected = int(text_a) * int(text_b)
    expected_str = str(expected)
    verdict = "MATCH" if product_str == expected_str else "MISMATCH"

    if verdict == "MISMATCH":
        print("MISMATCH!")
        print("Expected:", expected_str[:100])
        print("Got:     ", product_str[:100])

    print(verdict)

    # Count digits (ignoring sign)
    def digit_count(s):
        s = s.lstrip('-+').lstrip('0') or '0'
        return len(s)

    digits_a = digit_count(text_a)
    digits_b = digit_count(text_b)
    digits_prod = digit_count(product_str)
    ...
    report_lines = [
        ...
        "digits of product   : %d" % digits_prod,
