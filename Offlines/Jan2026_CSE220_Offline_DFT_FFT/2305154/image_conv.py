...
def transform_2d(plane, engine):
    ...
    # Transform every row
    result = np.empty((P, Q), dtype=np.complex128)
    for r in range(P):
        result[r, :] = engine.transform(plane[r, :])

    # Transform every column of the result
    for c in range(Q):
...
def convolve_plane(plane, kernel, engine, circular=False):
    ...
    if circular:
        # Circular convolution: transform at exactly (H, W), kernel wrapped
        # Place kernel at origin, wrap the part that hangs over
        kernel_padded = np.zeros((H, W), dtype=np.float64)
        # Kernel sits at origin; wrap around using np.roll approach
        # Place the kernel centered at origin (wrap any overflow)
        # kh//2 and kw//2 are offsets to shift kernel centre to (0,0)
        for ki in range(kh):
            for kj in range(kw):
                pi = (ki - kh // 2) % H
                pj = (kj - kw // 2) % W
                kernel_padded[pi, pj] += kernel[ki, kj]
        ...
        result = inverse_2d(F_result, engine).real
        ...
        plane_padded = np.zeros((pad_h, pad_w), dtype=np.float64)
        plane_padded[:H, :W] = plane

        # Kernel sits at origin of the padded array (NOT centered)
        ...
        # Inverse transform
        result_full = inverse_2d(F_result, engine).real

        # Crop: result is offset by (kh//2, kw//2) because kernel is at origin
        r_start = kh // 2
        c_start = kw // 2
        result = result_full[r_start:r_start + H, c_start:c_start + W]
...
def convolve_image(image, kernel, engine, circular=False):
    ...
    if image.ndim == 2:
        ...
        for c in range(image.shape[2]):
            planes.append(convolve_plane(image[:, :, c], kernel, engine, circular=circular))
...
def run_single(path, kernel_name, param, engine_name, out_dir, gray=False):
    ...
    # Linear (zero-padded) convolution
    blurred = convolve_image(image, kernel, engine, circular=False)

    # Circular (no-padding) convolution
    wraparound = convolve_image(image, kernel, engine, circular=True)

    # Determine the sizes for reporting
    # Linear convolution output size (before cropping)
    linear_h = H + kh - 1
    linear_w = W + kw - 1
    ...
    # Verification: convolve top-left 64x64 corner both ways
    corner_size = 64
    if image.ndim == 2:
        corner = image[:corner_size, :corner_size]
    else:
        corner = image[:corner_size, :corner_size, 0]  # first colour plane

    spectral_corner = convolve_plane(corner, kernel, engine, circular=False)
    direct_corner = convolve_plane_direct(corner, kernel)
    verification_error = np.max(np.abs(spectral_corner - direct_corner))
    verdict = "OK (%.2e)" % verification_error
    if verification_error > 1e-9:
        verdict = "FAIL (%.2e) -- BUG, not rounding!" % verification_error

    print("Verification error (top-left 64x64): %.2e" % verification_error)
    ...
    save_kernel_preview(kernel, os.path.join(out_dir, "kernel.png"),
                        title="%s (param=%s)" % (kernel_name, param))
    save_comparison(
    ...
    # Determine color mode string
    color_mode = "gray" if gray else "RGB"

    # Write report.txt -- format matches expected_outputs exactly
    verify_verdict = "MATCH" if verification_error <= 1e-9 else "MISMATCH"
    report_lines = [
        "Task B -- 2D convolution through the frequency domain",
        "image               : %s  (%d x %d, %s)" % (path, H, W, color_mode),
        "kernel              : %s  (%d x %d)" % (kernel_name, kh, kw),
        "engine              : %s" % engine_name,
        "linear-conv size    : %d x %d" % (linear_h, linear_w),
        "transform size      : %d x %d" % (transform_h, transform_w),
        "max |spectral - direct| on 64x64 crop : %.3e" % verification_error,
        "verification        : %s" % verify_verdict,
    ]
